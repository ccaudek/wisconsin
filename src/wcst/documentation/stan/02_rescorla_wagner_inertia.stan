...
model {
  // ---------------------- PRIORS ----------------------
  // Group-level priors
  mu_alpha ~ beta(2, 2); // or something else if you prefer
  mu_beta ~ normal(0, 2); // broad prior
  mu_inertia ~ normal(0, 2); // broad prior for positivity
  
  sigma_alpha ~ normal(0, 0.5); // half-normal
  sigma_beta ~ normal(0, 0.5); // half-normal
  sigma_inertia ~ normal(0, 0.5); // half-normal
