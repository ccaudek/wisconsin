...
get_one_subj_data_for_stan <- function(df, subj_name) {
  ...
  df_subj <- df_subj %>%
    ...
    dplyr::select(
      rew, los, rule_choice, resp_choice, resp_color, resp_shape, resp_number
    )
...
generate_csv <- function(group) {
  ...
  file_path <- here("src", "wcst", "data", paste0("raw_data_", group, ".csv"))
...
process_and_prepare_stan_data <- function(DEBUGGING = FALSE) {
  ...
  # Read raw data
  raw_data_patients_df <- rio::import(here::here("src", "wcst", "data", "raw_data_patients.csv"))
  raw_data_controls_df <- rio::import(here::here("src", "wcst", "data", "raw_data_controls.csv"))
