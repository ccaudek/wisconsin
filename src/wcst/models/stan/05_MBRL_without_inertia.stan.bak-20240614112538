...
data {
  ...
  array[N] int<lower=0, upper=1> holdout; // Should the participant be holdout for cross validation?
...
transformed parameters {
  ...
  for (i in 1 : N) {
    ...
    temp[i] = Phi_approx(mu_p[3] + sigma[3] * temp_pr[i]) * 5;
  }
}
model {
  // Hyperparameters
