...
# Complete list of the 50 patients who are included in this project.
...
# From the 60 patiens that participanted in the study, in this project we keep
# 50. The list of the 50 patients is participants_list[[1]]. Not all 50
# patients completed each of the three tasks: WCST, PRL, task switching.
# Here I select the patients who completed the PRL task from the original 60.
patients_keep <- 
  patients_wcst_look_up_tbl[
    patients_wcst_look_up_tbl$subj_name %in% patients_from_prl, ]
# This produces 45 patients who completed the WCST.

# These are the 19 patients who completed the WCST but did not complete the 
# PRL task.
patients_not_keep <- 
  patients_wcst_look_up_tbl[
    !(patients_wcst_look_up_tbl$subj_name %in% patients_from_prl), ]
...
df_patients <- readRDS(
  here::here("data", "interim", "input_stan_patients.RDS")
)
length(unique(df_patients$subj_name))
# 60
