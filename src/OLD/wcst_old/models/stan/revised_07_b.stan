...
parameters {
  ...
  vector[K] mu_p;
  vector<lower=0>[K] sigma_param; // std dev for each param at population level
...
model {
  // Priors
  mu_p ~ normal(0, 1); // Moderatamente informativo, lascia spazio a differenze
  sigma_param ~ normal(0, 1); // Consente una variabilità ragionevole senza essere troppo stretta
  // Se necessario, si può anche usare half-Cauchy o half-normal più ampio, ma normal(0,1) dovrebbe essere un buon compromesso.
